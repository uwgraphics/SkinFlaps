...
//=======================================================
    ...
    for (int __a = 0; __a < 9; __a++)
      for (int __b = 0; __b < 16; __b++)
        B[__a][__b] = Get_Random < T >();
    for (int __a = 0; __a < 9; __a++)
      for (int __b = 0; __b < 16; __b++)
      {
        C_original[__a][__b] = Get_Random < T >();
...
//=======================================================
    ...
    T               __mA[9] __attribute__ ((aligned (8)));
    T               __mB[9] __attribute__ ((aligned (8)));
    T               __mC[9] __attribute__ ((aligned (8)));
    T               __mC_reference[9] __attribute__ ((aligned (8)));
    T               __mC_original[9] __attribute__ ((aligned (8)));
    for (int k = 0; k < 16; k++)
    {
      for (int __a = 0; __a < 9; __a++)
        __mA[__a] = A[__a][k];
      ...
      Matrix_Times_Transpose < SIMDArchitectureScalar<T>, T >(__mA, __mB, __mC_reference);
      for (int __a = 0; __a < 9; __a++)
...
//=======================================================

    {
      ...
      for (int i = 0; i < 16; i += 1)
      {
        ...
        Matrix_Times_Transpose < SIMDArchitectureScalar<T>, T[16]> (Ak, Bk, Ck);
      }
      for (int __a = 0; __a < 9; __a++)
        for (int __b = 0; __b < 16; __b++)
          if (std::
...
//=======================================================
    ...
    {
        std::cout << "Running AVX2 Test for Matrix_Times_Transpose " << std::endl;
      typedef         T (&refArray1)[9][16];
      typedef         T (&refArray2)[9][16];
      typedef         T (&refArray3)[9][16];
      for (int __a = 0; __a < 9; __a++)
      ...
      for (int i = 0; i < 16; i += SIMDArchitectureAVX2<T>::Width)
      {
        ...
        refArray2       Bk = reinterpret_cast < refArray2 > (B[0][i]);
        refArray3       Ck = reinterpret_cast < refArray3 > (C[0][i]);
        Matrix_Times_Transpose < SIMDArchitectureAVX2<T>, T[16] > (Ak, Bk, Ck);
      }
      for (int __a = 0; __a < 9; __a++)
        for (int __b = 0; __b < 16; __b++)
          if (std::
...
//=======================================================
    ...
    {
        std::cout << "Running AVX512 Test for Matrix_Times_Transpose " << std::endl;
      typedef         T (&refArray1)[9][16];
      typedef         T (&refArray2)[9][16];
      typedef         T (&refArray3)[9][16];
      for (int __a = 0; __a < 9; __a++)
      ...
      for (int i = 0; i < 16; i += 16)
      {
        ...
        refArray2       Bk = reinterpret_cast < refArray2 > (B[0][i]);
        refArray3       Ck = reinterpret_cast < refArray3 > (C[0][i]);
        Matrix_Times_Transpose < SIMDArchitectureAVX512<T>, T[16]> (Ak, Bk, Ck);
      }
      for (int __a = 0; __a < 9; __a++)
        for (int __b = 0; __b < 16; __b++)
          if (std::
