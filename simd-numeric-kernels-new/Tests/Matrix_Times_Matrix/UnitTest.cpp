...
main (int argc, char *argv[])
{
...

  {
    ...
    for (int i = 0; i < 1; i += 1)
      {
        Matrix_Times_Matrix < SIMDArchitectureScalar<T>, T>(A, B, C);
      }

    Matrix_Times_Matrix_Reference < T >(A, B, C_reference);
    if (!(Matrix_Times_Matrix_Compare < T >(C, C_reference)))
      {
        std::
          cout << "Failed to confirm unit test for Matrix_Times_Matrix " <<
          std::endl;
        return 1;
      }
