...
template<class T> T PhysBAM::TETRAHEDRON<T>::
Minimum_Altitude(int& min_altitude_vertex) const
{
    VECTOR<T,4> distance;distance(1)=abs(triangle1.Signed_Distance(X[4]));distance(2)=abs(triangle2.Signed_Distance(X[3]));distance(3)=abs(triangle3.Signed_Distance(X[2]));distance(4)=abs(triangle4.Signed_Distance(X[1]));
    min_altitude_vertex=1;T min_altitude=distance(1);for(int i=2;i<=4;i++)if(distance(i)<min_altitude){min_altitude=distance(i);min_altitude_vertex=i;}return min_altitude;
}
//#####################################################################
...
{  
    return acos(clamp(-max(VECTOR<T,3>::Dot_Product(triangle1.normal,triangle2.normal),VECTOR<T,3>::Dot_Product(triangle1.normal,triangle3.normal),
                               VECTOR<T,3>::Dot_Product(triangle1.normal,triangle4.normal),VECTOR<T,3>::Dot_Product(triangle2.normal,triangle3.normal),
                               VECTOR<T,3>::Dot_Product(triangle2.normal,triangle4.normal),VECTOR<T,3>::Dot_Product(triangle3.normal,triangle4.normal)),(T)-1,(T)1));
}
//#####################################################################
