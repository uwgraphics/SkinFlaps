...
//#####################################################################
template<class TV> RANGE<TV> POLYGON<TV>::Bounding_Box() const
{
    RANGE<TV> bounding_box=RANGE<TV>::Empty_Box();
    for(int i=1;i<=X.Size();i++)
        bounding_box.Enlarge_To_Include_Point(X(i));
    return bounding_box;
}
//#####################################################################
// Function Normal
//#####################################################################
template<class TV> TV POLYGON<TV>::Normal(const TV& X_point) const
{
    int side;Find_Closest_Point_On_Polygon(X_point,side);
    TV side_vector=X(side+1>X.Size()?1:side+1)-X(side);
    if(TV::dimension==1 || TV::dimension==3) return TV();
    else{TV normal;normal(1)=side_vector(2);normal(2)=-side_vector(1);normal=normal.Normalized();return normal;}
}
//#####################################################################
namespace PhysBAM {
