...
Fast_Marching_Method(T_ARRAYS_SCALAR& phi_ghost,const T stopping_distance,const ARRAY<TV_INT>* seed_indices,const bool add_seed_indices_for_ghost_cells,const bool add_extra_fixed_seed_indices,const bool update_ghost_cells)
{
    /*int heap_length=0;
    ...
    threaded_iterator.template Run<T_ARRAYS_SCALAR&,ARRAY<T_ARRAYS_SCALAR>&,const T,const ARRAY<TV_INT>*,const bool,const bool,const bool>(*this,&FAST_MARCHING_METHOD_UNIFORM<T_GRID>::Fast_Marching_Method_Threaded_Part_Two,phi_ghost,phi_new_array,stopping_distance,seed_indices,add_seed_indices_for_ghost_cells,add_extra_fixed_seed_indices,update_ghost_cells);
}
//#####################################################################
// Function Fast_Marching_Method
//#####################################################################
...
template<class T_GRID> void FAST_MARCHING_METHOD_UNIFORM<T_GRID>::
Fast_Marching_Method_Threaded_Part_Two(int tid,RANGE<TV_INT>& domain,T_ARRAYS_SCALAR& phi_ghost,ARRAY<T_ARRAYS_SCALAR>& phi_new_array,const T stopping_distance,const ARRAY<TV_INT>* seed_indices,const bool add_seed_indices_for_ghost_cells,const bool add_extra_fixed_seed_indices,const bool update_ghost_cells)
{
    int heap_length=0;
    ...
    Initialize_Interface(domain,phi_new,done,close_k,heap,heap_length,seed_indices,add_seed_indices_for_ghost_cells,add_extra_fixed_seed_indices);
    RANGE<TV_INT> interior_domain=domain.Thickened(-ghost_cells);
    if(update_ghost_cells){
        for(int axis=1;axis<=TV::dimension;axis++){
            if(interior_domain.min_corner(axis)==cell_grid.Domain_Indices().min_corner(axis)) interior_domain.min_corner(axis)-=ghost_cells;
            if(interior_domain.max_corner(axis)==cell_grid.Domain_Indices().max_corner(axis)) interior_domain.max_corner(axis)+=ghost_cells;}}
...
template<class T_GRID> void FAST_MARCHING_METHOD_UNIFORM<T_GRID>::
Initialize_Interface(RANGE<TV_INT>& domain,T_ARRAYS_SCALAR& phi_ghost,T_ARRAYS_BOOL& done,T_ARRAYS_INT& close_k,ARRAY<TV_INT>& heap,int& heap_length,const ARRAY<TV_INT>* seed_indices,const bool add_seed_indices_for_ghost_cells,const bool add_extra_fixed_seed_indices)
{ 
    RANGE<TV_INT> interior_domain=domain.Thickened(-ghost_cells);
    if(seed_indices){
    ...
    if(!seed_indices || add_extra_fixed_seed_indices){
        if(levelset.collision_body_list){
            for(int axis=1;axis<=TV::dimension;axis++){
