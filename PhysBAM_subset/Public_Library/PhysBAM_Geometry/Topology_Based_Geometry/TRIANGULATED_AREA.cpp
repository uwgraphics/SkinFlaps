...
Average_Area() const
{
    ...
    for(int t=1;t<=mesh.elements.m;t++){
        int node1=mesh.elements(t)(1),node2=mesh.elements(t)(2),node3=mesh.elements(t)(3);
        average+=TRIANGLE_2D<T>::Area(particles.X(node1),particles.X(node2),particles.X(node3));}
    if(mesh.elements.m) return average/mesh.elements.m;
    else return average;
}
//#####################################################################
...
Barycentric_Interpolate(const VECTOR<T,2>& x,ARRAY_VIEW<T2> values,T2& result,int* index)const
{
    ...
    if(index) *index=element_index;
    return true;
}
//#####################################################################
namespace PhysBAM {
template class TRIANGULATED_AREA<float>;
template bool TRIANGULATED_AREA<float>::Barycentric_Interpolate(const VECTOR<float,2>&,ARRAY_VIEW<float>,float&,int* index)const;
template bool TRIANGULATED_AREA<float>::Barycentric_Interpolate(const VECTOR<float,2>&,ARRAY_VIEW<VECTOR<float,2> >,VECTOR<float,2>&,int* index)const;
#ifndef COMPILE_WITHOUT_DOUBLE_SUPPORT
template class TRIANGULATED_AREA<double>;
template bool TRIANGULATED_AREA<double>::Barycentric_Interpolate(const VECTOR<double,2>&,ARRAY_VIEW<double>,double&,int* index)const;
template bool TRIANGULATED_AREA<double>::Barycentric_Interpolate(const VECTOR<double,2>&,ARRAY_VIEW<VECTOR<double,2> >,VECTOR<double,2>&,int* index)const;
#endif
}
