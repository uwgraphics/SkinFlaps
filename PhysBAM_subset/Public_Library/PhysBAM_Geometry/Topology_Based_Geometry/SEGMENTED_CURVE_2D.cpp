...
Total_Volume()const
{
    // assume a single closed loop
    const ARRAY_VIEW<TV> X(particles.X);
    const ARRAY<VECTOR<int,2> >& elements=mesh.elements;
    T total_volume=0;
    TV center;for(int i=1;i<=elements.m;i++) center+=SEGMENT_2D<T>::Center(X.Subset(elements(i)));center/=(typename TV::SCALAR)elements.m;
    for(int i=1;i<=elements.m;i++) total_volume+=TRIANGLE_2D<T>::Signed_Size(VECTOR<TV,3>(center,X(elements(i).x),X(elements(i).y)));
    return total_volume;
}
//#####################################################################
...
//#####################################################################
template<class T> void SEGMENTED_CURVE_2D<T>::
Initialize_Conical_Frustum_Mesh_And_Particles(const T length,const T radius1,const T radius2,const bool create_caps)
{
    TOPOLOGY_BASED_GEOMETRY_COMPUTATIONS::Initialize_Conical_Frustum_Mesh_And_Particles(*this,length,radius1,radius2,create_caps);
}
