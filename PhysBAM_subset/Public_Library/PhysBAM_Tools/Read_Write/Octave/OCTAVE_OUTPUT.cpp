...
{
    SPARSE_MATRIX_FLAT_MXN<T> t,t_compressed;
    m.Transpose(t);
    t.Compress(t_compressed);
    Write_Transpose(name,t_compressed);
}
//#####################################################################
...
template void OCTAVE_OUTPUT<float>::Write(const char* name,const MATRIX_BASE<float,MATRIX_MXN<float> >& m);
...
template void OCTAVE_OUTPUT<double>::Write(const char* name,const MATRIX_BASE<double,MATRIX_MXN<double> >& m);
#endif
}
