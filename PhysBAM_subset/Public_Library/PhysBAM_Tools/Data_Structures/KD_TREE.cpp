...
Median_Split(const int partition_index,const int first_index,const int last_index,ARRAY_VIEW<const TV> points,ARRAY_VIEW<int> permutation_array,const int axis)
{
    ...
    int* middle=std::partition(permutation_subset.begin(),permutation_subset.end(),partition_helper_less);
    std::partition(middle,permutation_subset.end(),partition_helper_less_equal);
