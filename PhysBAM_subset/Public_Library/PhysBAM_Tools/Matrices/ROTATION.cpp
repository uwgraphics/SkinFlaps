//#####################################################################
// Copyright 2009-2014, Craig Schroeder, Rahul Sheth.
...
//#####################################################################
template<class T>  ROTATION<VECTOR<T,3> > ROTATION<VECTOR<T,3> >::
From_Rotation_Matrix(const MATRIX<T,3>& R)
{
    T q0=sqrt(clamp_min((R(1,1)+R(2,2)+R(3,3)+(T)1.0)/(T)4.0,(T)0.0)),q1=sqrt(clamp_min((R(1,1)-R(2,2)-R(3,3)+(T)1.0)/(T)4.0,(T)0.0));
    T q2=sqrt(clamp_min((-R(1,1)+R(2,2)-R(3,3)+(T)1.0)/(T)4.0,(T)0.0)),q3=sqrt(clamp_min((-R(1,1)-R(2,2)+R(3,3)+(T)1.0)/(T)4.0,(T)0.0));
    if(q0>=q1&&q0>=q2&&q0>=q3){q1*=sign(R(3,2)-R(2,3));q2*=sign(R(1,3)-R(3,1));q3*=sign(R(2,1)-R(1,2));}
    else if(q1>=q0&&q1>=q2&&q1>=q3){q0*=sign(R(3,2)-R(2,3));q2*=sign(R(2,1)+R(1,2));q3*=sign(R(1,3)+R(3,1));}
    else if(q2>=q0&&q2>=q1&&q2>=q3){q0*=sign(R(1,3)-R(3,1));q1*=sign(R(2,1)+R(1,2));q3*=sign(R(3,2)+R(2,3));}
    else if(q3>=q0&&q3>=q1&&q3>=q2){q0*=sign(R(2,1)-R(1,2));q1*=sign(R(3,1)+R(1,3));q2*=sign(R(3,2)+R(2,3));}
    else{PHYSBAM_FATAL_ERROR("Could not convert from rotation matrix!");}
    VECTOR<T,4> quat(q0,q1,q2,q3);quat.Normalize();
    return ROTATION<TV>(QUATERNION<T>(quat));
}
//#####################################################################
